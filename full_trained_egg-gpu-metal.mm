...
static NSString *EggMetalShaderSource(void) {
    return [NSString stringWithFormat:
        @"#include <metal_stdlib>\n"
         ...
         "  x_norm[gid] = char(xn);\n"
         "  threadgroup_barrier(mem_flags::mem_threadgroup);\n"
         "  // Parallel reduction for xB1 and xB2\n"
         "  threadgroup int xB1_partial[HIDDEN_DIM];\n"
         "  threadgroup int xB2_partial[HIDDEN_DIM];\n"
         "  const device char *nb1 = noiseB_all + 0 * HIDDEN_DIM;\n"
         "  const device char *nb2 = noiseB_all + 1 * HIDDEN_DIM;\n"
         "  xB1_partial[tid] = int(x_norm[gid]) * int(nb1[gid]);\n"
         "  xB2_partial[tid] = int(h_vec[gid]) * int(nb2[gid]);\n"
         "  threadgroup_barrier(mem_flags::mem_threadgroup);\n"
         "  // Tree reduction for both xB1 and xB2\n"
         "  for (uint s = HIDDEN_DIM / 2; s > 0; s >>= 1) {\n"
         "    if (tid < s) {\n"
         "      xB1_partial[tid] += xB1_partial[tid + s];\n"
         "      xB2_partial[tid] += xB2_partial[tid + s];\n"
         "    }\n"
         "    threadgroup_barrier(mem_flags::mem_threadgroup);\n"
         "  }\n"
         "  if (tid == 0) {\n"
         "    shared_scalars[1] = xB1_partial[0];\n"
         "    shared_scalars[2] = xB2_partial[0];\n"
         "  }\n"
         "  threadgroup_barrier(mem_flags::mem_threadgroup);\n"
         ...
         "  threadgroup_barrier(mem_flags::mem_threadgroup);\n"
         "  // Parallel reduction for xB3 and xB4 (reuse partial arrays)\n"
         "  const device char *nb3 = noiseB_all + 2 * HIDDEN_DIM;\n"
         "  const device char *nb4 = noiseB_all + 3 * HIDDEN_DIM;\n"
         "  xB1_partial[tid] = int(x_norm[gid]) * int(nb3[gid]);\n"
         "  xB2_partial[tid] = int(gp_vec[gid]) * int(nb4[gid]);\n"
         "  threadgroup_barrier(mem_flags::mem_threadgroup);\n"
         "  // Tree reduction for both xB3 and xB4\n"
         "  for (uint s = HIDDEN_DIM / 2; s > 0; s >>= 1) {\n"
         "    if (tid < s) {\n"
         "      xB1_partial[tid] += xB1_partial[tid + s];\n"
         "      xB2_partial[tid] += xB2_partial[tid + s];\n"
