...
void matmul_perturbed(
    ...
    int32_t xB = dot_product_i8(in, B, cols);
...
Dataset load_data(const char *filename) {
    ...
    if(!data) {
        fclose(f);
        fprintf(stderr, "Error: failed to allocate %ld bytes for dataset.\n", len);
        exit(1);
    }
    size_t bytes_read = fread(data, 1, len, f);
    fclose(f);
    if (bytes_read != (size_t)len) {
        fprintf(stderr, "Error: failed to read full file '%s' (%zu/%ld bytes).\n", filename, bytes_read, len);
        free(data);
        exit(1);
    }
